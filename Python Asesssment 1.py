...
# Sample books
books = [
    Book("Python Programming", "John Doe", "123456", "A book on Python programming."),
    Book("Advanced Python", "Jane Doe", "789012", "A book on advanced Python concepts.")
]
...
def view_order_history():
    if not current_user:
        print("Please login first.")
        return
    if not current_user.order_history:
        print("Your order history is empty.")
        return
    print("Order History:")
    for book in current_user.order_history:
        print(f"Title: {book.title}, Author: {book.author}")
# Purchasing Books
def purchase_books():
    if not current_user:
...
#viewing and managing shopping basket
def add_to_basket():
    if not current_user:
        print("Please login first.")
        return
    book_title = input("Enter the book title you want to add to your basket: ")
